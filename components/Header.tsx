    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm">
      <div className="bg-[#0a3858] text-white">
        <div className="container-shell flex min-h-9 items-center justify-between gap-4 text-xs font-semibold">
          <span className="hidden sm:block">
            Precision tooling for the mechanical industry
          </span>
      ...
      <div className="border-b border-slate-200">
        <div className="container-shell flex min-h-[76px] items-center justify-between gap-5">
          ...
          <nav
            className="hidden items-center justify-center gap-5 xl:flex xl:flex-1 xl:gap-6"
              ...
              item.label === "Products" ? (
                <div key={item.href} className="relative">
                  <button
                    ...
                    className={`inline-flex items-center gap-1 py-7 text-sm font-bold ${pathname.startsWith("/products") ? "text-[#0a3858]" : "text-slate-700 hover:text-[#0a3858]"}`}
                    aria-expanded={productsOpen}
                  >
                    Products <ChevronDown className="h-4 w-4" />
                  </button>
                  {productsOpen && (
                    <div className="absolute left-0 top-full w-72 border border-slate-200 bg-white p-2 shadow-xl">
                      {productLinks.map((link) => (
                        <Link
                          key={link.href}
                          href={link.href}
                          className="block px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-[#eef4f8] hover:text-[#0a3858]"
                        >
                          {link.label}
                        </Link>
                      ))}
                ...
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative py-7 text-sm font-bold ${pathname === item.href ? "text-[#0a3858]" : "text-slate-700 hover:text-[#0a3858]"}`}
                >
                  {item.label}
                  {pathname === item.href && (
          ...
          <div className="hidden shrink-0 items-center gap-2 lg:flex">
            <a
              href={`tel:${company.phone}`}
              className="inline-flex min-h-11 items-center gap-2 border border-[#0a3858] px-4 text-sm font-bold text-[#0a3858] hover:bg-[#eef4f8]"
            >
              <Phone className="h-4 w-4" /> Call Us
            </a>
            <Link
              href="/contact"
              className="inline-flex min-h-11 items-center bg-[#0a3858] px-5 text-sm font-bold text-white hover:bg-[#082e49]"
            >
              Request an Enquiry
            </Link>
          </div>
              ...
              {navItems.map((item) =>
                item.label === "Products" ? (
                  <div key={item.href} className="py-2">
                    <button
                      type="button"
                      className="flex w-full items-center justify-between py-3 text-left text-base font-bold text-[#0a3858]"
                      onClick={() => setProductsOpen((v) => !v)}
                    >
                      Products{" "}
                      <ChevronDown
                    ...
                    {productsOpen && (
                      <div className="border-l-2 border-amber-400 pl-4 pb-2">
                        {productLinks.map((link) => (
                          <Link
                            key={link.href}
                            href={link.href}
                  ...
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`block py-4 text-base font-bold ${pathname === item.href ? "border-l-4 border-amber-400 pl-4 text-[#0a3858]" : "text-slate-700"}`}
                  >
                    {item.label}
                  </Link>
                ),
